...
class chi2:
    ...
    def read_input(self):
        #read vorbin file
        dp = pd.read_csv("C:/Users/marti/Desktop/share/vorbin_chi2/sn={}".format(self.sn))
        self.xBar = dp['xBar'].values
        self.yBar = dp['yBar'].values
        #read observed bin counts
        dp = pd.read_csv("C:/Users/marti/Desktop/share/vorbin_chi2/bin_count_std_sn={}".format(self.sn))
    ...
    def search_vorbin(self):
        ...
        Tb_size = 1000
        dms = np.linspace(14.62,14.82,1)
        reds = np.linspace(0.0,0.05,1)
        ages = ['09000','10000','11000','12000','13000','14000','15000','16000']
        chi2 = []
        for age in ages:
            #read iso files
            dp = pd.read_csv("C:/Users/marti/Desktop/share/dm_test/mc{}.a{}".format(self.mc_num,age),sep='\s+',names=['vi','v','i'],skiprows=3)
            for dm in dms:
                for red in reds:
        ...
        #do the last bit
                    ...
                    for j in range(total_pt - n_div*Tb_size):
                        bin_count[bin_num[j]] += 1
                    chi2.append([age, dm, red, np.inner(np.divide(bin_count,self.bin_count_std*(total_pt/self.obs_size)) - 1, bin_count - self.bin_count_std*(total_pt/self.obs_size))])
        self.chi2 = chi2
    def writeout(self):
        #write chi2 to csv file
        dp = pd.DataFrame(data=self.chi2,columns=['age','dm','red','chi2'])
        cwd = os.getcwd()
        path = cwd + "/outchi2/chi2_mc{}_sn{}".format(self.mc_num,self.sn)
        dp.to_csv(path)
