...
class chi2:
    ...
    def read_input(self):
        ...
        self.obs_data = pd.read_csv('C:/Users/marti/Desktop/school work/Dartmouth/M92 project/M92-20211208T215351Z-001/M92/ObservationalData/M92_fitstars.dat',sep='\s+',names=['v','i','vi'],skiprows=3)
    ...
    def search_vorbin(self):
        ...
        for age in ages:
            ...
            dp = dp[(dp['vi'] < (obs_vi_max + red_min)) & (dp['vi'] > (obs_vi_min + red_max))& (dp['v'] < (obs_v_max + dm_min)) & (dp['v'] > (obs_v_min + dm_max))]
            #generate vorbin use the first 180000 data points
            x = dp['v'].values[:180000]
            y = dp['vi'].values[:180000] * 12.5
            signal = np.array([1]*180000)
            noise = np.array([1]*180000)
            targetSN = self.sn
            binNum, xNode, yNode, xBar, yBar, sN, nPixels, scale = voronoi_2d_binning.voronoi_2d_binning(x, y, signal, noise, targetSN, plot=0, quiet=1)
            ...
            for i in range(len(bin_count_std)):
                if bin_count_std[i] == 0:
                    bin_count_std[i] += 1
            writevorbin(xBar, yBar, bin_count_std, age)
            #search through observed data
            for dm in dms:
                for red in reds:
                    bin_count = np.zeros(len(xBar))
                    total_pt = len(self.obs_data)
                    n_div = total_pt // Tb_size
                    for i in range(n_div):
                        bin_num = self.search_point_location_bc((self.obs_data['vi'].values[i*Tb_size:(i+1)*Tb_size] + red)*12.5, self.obs_data['v'].values[i*Tb_size:(i+1)*Tb_size] + dm, xBar, yBar)
                        for j in range(Tb_size):
                            bin_count[bin_num[j]] += 1
            #do the last bit
                    bin_num = self.search_point_location_bc((self.obs_data['vi'].values[n_div*Tb_size:] + red)*12.5, self.obs_data['v'].values[n_div*Tb_size:] + dm, xBar, yBar)
                    for j in range(total_pt - n_div*Tb_size):
                        bin_count[bin_num[j]] += 1
                    #calculate chi2
                    chi2.append([age, dm, red, np.inner(np.divide(self.bin_count,bin_count_std/(total_pt/self.obs_size)) - 1, self.bin_count - bin_count_std/(total_pt/self.obs_size))])
        self.chi2 = chi2
    ...
    def writevorbin(self, xBar, yBar, bin_count_std, age):
        ...
        path = "/data/outchi2/bin_mc{}_sn{}.age{}".format(self.mc_num,self.sn,age)
        dp.to_csv(path)
