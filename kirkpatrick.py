...
from polygons import generate_random_tiling, generate_triangle_tiling
...
def remove_point_triangulation(affected_triangles: List[Triangle], p: Point) -> Polygon:
    ...
    graph = Graph()
    #print(point2triangles.items()[0])
    #del_key = []
    #for item in point2triangles.items():
    #    if len(item[1]) != 2:
    #        print(item[1])
    #        del_key.append(item[0])
    #for key in del_key:
    #    del point2triangles[key]
...
class Kirkpatrick:
    ...
    def locate(self, p: Point, plot_search=False) -> Optional[Polygon]:
        ...
        if self.digraph._node[curr]["original"]:
            return curr
        return None
